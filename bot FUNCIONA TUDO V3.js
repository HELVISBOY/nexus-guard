...
process.on('unhandledRejection', (error) => {
    if (error.message && (error.message.includes('description') || error.message.includes('getLastMsgKeyForAction'))) {
...
process.on('uncaughtException', (error) => {
    ...
    console.log('⚠️ Erro crítico:', error.message);
});
// ════════════════════════════════════════════════════════
...
const CONFIG = {
    ...
    MEU_NOME: 'Helvis Dev. Full Stak',
    
    // IA - GROQ
    GROQ_API_KEY: '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', // ← COLA TUA CHAVE AQUI!
    IA_ENABLED: true,
    IA_NAME: 'NEXUS',
    ...
    AUTO_STICKER: true,
    WORD_FILTER: true,
    bad_words: ['porra', 'caralho', 'fdp'], // Adicione mais se quiser
...
if (CONFIG.IA_ENABLED && CONFIG.GROQ_API_KEY && CONFIG.GROQ_API_KEY.length > 20) {
    ...
    console.log('🧠 IA GROQ inicializada! (Modelo: llama-3.3-70b)');
}
...
const getUserStats = (userId, groupId) => {
    const key = `${groupId}_${userId}`;
    return userStats[key] || { xp: 0, level: 1, coins: 0, messages: 0, lastDaily: 0 };
};

// IA - grok
const askAI = async (question) => {
    if (!groq) return '❌ IA não configurada. Configure a API Key do GROQ.';
    ...
    try {
        const completion = await groq.chat.completions.create({
            messages: [
                {
                    role: "system",
                    content: `Você é ${CONFIG.IA_NAME}, um assistente inteligente e prestativo criado por ${CONFIG.MEU_NOME}. Responda de forma objetiva e amigável em português do Brasil.`
                },
                {
                    role: "user",
                    content: question
        ...
        return '⚠️ Desculpe, tive um erro ao processar sua pergunta.';
    }
};
...
╚═══════════════════╝\n\n`;
        ...
        if (command === 'comandos' || command === 'help' || command === 'menu') {
            if (isAdminUser) {
                // SÓ VOCÊ VÊ ISSO (admin completo)
                await message.reply(`╔═══════════════════════════════╗
...
⚡ NEXUS GUARD - Powered by AI`);
                ...
                // OUTROS USUÁRIOS VEEM ISSO (limitado)
                await message.reply(`╔═══════════════════╗
║  🤖 NEXUS GUARD  ║
╚═══════════════════╝
